  ...
  constructor(buffer: Buffer) {
    ...
    this.endianess('little')
        ...
        .array(
            ...
            {length: 4, type: new Parser().floatle('m_suspensionPosition')})
        .array(
            'm_suspensionVelocity',
            {length: 4, type: new Parser().floatle('m_suspensionVelocity')})
        .array(
            'm_suspensionAcceleration',
            {length: 4, type: new Parser().floatle('m_suspensionAcceleration')})
        .array(
            'm_wheelSpeed',
            {length: 4, type: new Parser().floatle('m_wheelSpeed')})
        .array(
            'm_wheelSlip',
            {length: 4, type: new Parser().floatle('m_wheelSlip')})
