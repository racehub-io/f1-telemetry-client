...
export class CarTelemetryDataParser extends F1Parser {
  constructor(packetFormat: number) {
    super();
    (this as any).uint16le('m_speed');

    if (packetFormat === 2018) {
      (this as any).uint8('m_throttle').int8('m_steer').uint8('m_brake');
    } else if (packetFormat === 2019 || packetFormat === 2020) {
      (this as any).floatle('m_throttle').floatle('m_steer').floatle('m_brake');
    }
    ...
    if (packetFormat === 2019 || packetFormat === 2020) {
      (this as any).array('m_surfaceType', {
        length: 4,
